...
typedef struct {
  // GLYPH CACHE. Every glyph in the ASCII range is cached when first used; no additional memory aside from the one needed to render the character set.
...
void *readFile(const char *path, int align, int *size) {
  ...
  if (fd >= 0) {
    ...
    if ((*size > 0) && (*size != realSize)) {
      printf("fntsys: readFile: invalid filesize, expected: %d, got: %d\n", *size, realSize);
      close(fd);
      return NULL;
    }
    ...
    if (!buffer) {
      printf("fntsys: readFile: failed allocation of %d bytes\n", realSize);
      *size = 0;
    } else {
...
static void fntPrepareCLUT() {
  ...
  fontClut.Clut = (uint32_t *)memalign(128, 256 * 4);
  fontClut.VramClut = 0;

  // generate the clut table
  size_t i;
  uint32_t *clut = fontClut.Clut;
  for (i = 0; i < 256; ++i) {
...
static int fntLoadSlot(font_t *font, const char *path) {
  ...
  printf("fntsys: loaded font %s\n", path);

  return 0;
}

static int fntLoadSlotFromMemory(font_t *font, const void *buf, int size) {
  fntInitSlot(font);
  if (!buf || size <= 0)
    return FNT_ERROR;
  int error = FT_New_Memory_Face(font_library, (const FT_Byte *)buf, size, 0, &font->face);
  if (error) {
    printf("fntsys: FreeType font from memory failed with %x\n", error);
    return FNT_ERROR;
  }
  font->dataPtr = NULL;
  font->isValid = 1;
...
int fntLoadFromMemory(const void *buf, int size) {
  for (int i = 0; i < FNT_MAX_COUNT; i++) {
    if (!fonts[i].isValid) {
      if (fntLoadSlotFromMemory(&fonts[i], buf, size) == 0)
        return i;
      return FNT_ERROR;
    }
  }
...
static int fntGlyphAtlasPlace(font_t *font, fnt_glyph_cache_entry_t *glyph, FT_GlyphSlot slot) {
  if (!slot)
    return 1;  // no slot - treat as empty glyph, skip placement
    ...
    if (!*atl) { // atlas slot not yet used
      *atl = fntNewAtlas();
      if (!*atl || !(*atl)->surface.Mem) {
        printf("fntsys: atlas alloc failed\n");
        return 0;
      }
    }
...
static fnt_glyph_cache_entry_t *fntCacheGlyph(font_t *font, uint32_t gid) {
  ...
  // Get slot once after FT_Load_Char; avoid reading face->glyph again (can be NULL for some glyphs)
  FT_GlyphSlot slot = font->face->glyph;
  if (!slot) {
    printf("fntsys: slot (face->glyph) is NULL, gid=%u\n", (unsigned)gid);
    return NULL;
  }

  // find atlas placement for the glyph
  if (!fntGlyphAtlasPlace(font, glyph, slot))
    return NULL;
...
void fntUpdateAspectRatio() {
  ...
  for (i = 0; i < FNT_MAX_COUNT; i++) {
    if (fonts[i].isValid && fonts[i].face) {
...
static inline int fntIdValid(int id) { return id >= 0 && id < FNT_MAX_COUNT && fonts[id].isValid; }

void fntSetPixelSize(int fontid, int width, int height) {
  if (!fntIdValid(fontid))
    return;
...
static void fntRenderGlyph(fnt_glyph_cache_entry_t *glyph, int pen_x, int pen_y) {
  // only if glyph has atlas placement (atlas must be non-NULL to avoid NULL+offset load)
    ...
    return;
  {
    ...
    // We should prepare a special fnt render method that would step over most of the performance
    // problems under - beginning with rmSetupQuad and continuing into gsKit - this method would
    // handle the preparation of the quads and GS upload itself, without the use of prim_quad_texture and rmSetupQuad...
...
#else
static int isRTL(uint32_t character) {
  return (((character >= 0x00000590 && character <= 0x000008FF) || (character >= 0x0000FB50 && character <= 0x0000FDFF) ||
           (character >= 0x0000FE70 && character <= 0x0000FEFF) || (character >= 0x00010800 && character <= 0x00010FFF) ||
           (character >= 0x0001E800 && character <= 0x0001EFFF))
              ? 1
              : 0);
}

static int isWeak(uint32_t character) {
  return (((character >= 0x0000 && character <= 0x0040) || (character >= 0x005B && character <= 0x0060) ||
           (character >= 0x007B && character <= 0x00BF) || (character >= 0x00D7 && character <= 0x00F7) ||
           (character >= 0x02B9 && character <= 0x02FF) || (character >= 0x2000 && character <= 0x2BFF))
              ? 1
              : 0);
}
