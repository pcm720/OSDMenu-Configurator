...
void init3D(float aspect) { create_view_screen(view_screen, aspect, -0.20f, 0.20f, -0.20f, 0.20f, 1.00f, 2000.00f); }
...
model *loadOBJ(const char *path, GSTEXTURE *text) {
  ...
  for (int i = 0; i < res_m->facesCount; i++) {
    if (lowX > res_m->positions[res_m->idxList[i]][0])
      lowX = res_m->positions[res_m->idxList[i]][0];
    if (hiX < res_m->positions[res_m->idxList[i]][0])
      hiX = res_m->positions[res_m->idxList[i]][0];
    if (lowY > res_m->positions[res_m->idxList[i]][1])
      lowY = res_m->positions[res_m->idxList[i]][1];
    if (hiY < res_m->positions[res_m->idxList[i]][1])
      hiY = res_m->positions[res_m->idxList[i]][1];
    if (lowZ > res_m->positions[res_m->idxList[i]][2])
      lowZ = res_m->positions[res_m->idxList[i]][2];
    if (hiZ < res_m->positions[res_m->idxList[i]][2])
      hiZ = res_m->positions[res_m->idxList[i]][2];
  }
...
void draw_bbox(model *m, float pos_x, float pos_y, float pos_z, float rot_x, float rot_y, float rot_z, Color color) {
  ...
  gsKit_prim_line_3d(gsGlobal, (t_xyz[0].x + 1.0f) * fX, (t_xyz[0].y + 1.0f) * fY, xyz[0].z, (t_xyz[1].x + 1.0f) * fX, (t_xyz[1].y + 1.0f) * fY,
                     xyz[1].z, color);
  gsKit_prim_line_3d(gsGlobal, (t_xyz[1].x + 1.0f) * fX, (t_xyz[1].y + 1.0f) * fY, xyz[1].z, (t_xyz[3].x + 1.0f) * fX, (t_xyz[3].y + 1.0f) * fY,
                     xyz[3].z, color);
  gsKit_prim_line_3d(gsGlobal, (t_xyz[2].x + 1.0f) * fX, (t_xyz[2].y + 1.0f) * fY, xyz[2].z, (t_xyz[3].x + 1.0f) * fX, (t_xyz[3].y + 1.0f) * fY,
                     xyz[3].z, color);
  gsKit_prim_line_3d(gsGlobal, (t_xyz[0].x + 1.0f) * fX, (t_xyz[0].y + 1.0f) * fY, xyz[0].z, (t_xyz[2].x + 1.0f) * fX, (t_xyz[2].y + 1.0f) * fY,
                     xyz[2].z, color);
  gsKit_prim_line_3d(gsGlobal, (t_xyz[4].x + 1.0f) * fX, (t_xyz[4].y + 1.0f) * fY, xyz[4].z, (t_xyz[5].x + 1.0f) * fX, (t_xyz[5].y + 1.0f) * fY,
                     xyz[5].z, color);
  gsKit_prim_line_3d(gsGlobal, (t_xyz[5].x + 1.0f) * fX, (t_xyz[5].y + 1.0f) * fY, xyz[5].z, (t_xyz[7].x + 1.0f) * fX, (t_xyz[7].y + 1.0f) * fY,
                     xyz[7].z, color);
  gsKit_prim_line_3d(gsGlobal, (t_xyz[6].x + 1.0f) * fX, (t_xyz[6].y + 1.0f) * fY, xyz[6].z, (t_xyz[7].x + 1.0f) * fX, (t_xyz[7].y + 1.0f) * fY,
                     xyz[7].z, color);
  gsKit_prim_line_3d(gsGlobal, (t_xyz[4].x + 1.0f) * fX, (t_xyz[4].y + 1.0f) * fY, xyz[4].z, (t_xyz[6].x + 1.0f) * fX, (t_xyz[6].y + 1.0f) * fY,
                     xyz[6].z, color);
  gsKit_prim_line_3d(gsGlobal, (t_xyz[0].x + 1.0f) * fX, (t_xyz[0].y + 1.0f) * fY, xyz[0].z, (t_xyz[4].x + 1.0f) * fX, (t_xyz[4].y + 1.0f) * fY,
                     xyz[4].z, color);
  gsKit_prim_line_3d(gsGlobal, (t_xyz[1].x + 1.0f) * fX, (t_xyz[1].y + 1.0f) * fY, xyz[1].z, (t_xyz[5].x + 1.0f) * fX, (t_xyz[5].y + 1.0f) * fY,
                     xyz[5].z, color);
  gsKit_prim_line_3d(gsGlobal, (t_xyz[2].x + 1.0f) * fX, (t_xyz[2].y + 1.0f) * fY, xyz[2].z, (t_xyz[6].x + 1.0f) * fX, (t_xyz[6].y + 1.0f) * fY,
                     xyz[6].z, color);
  gsKit_prim_line_3d(gsGlobal, (t_xyz[3].x + 1.0f) * fX, (t_xyz[3].y + 1.0f) * fY, xyz[3].z, (t_xyz[7].x + 1.0f) * fX, (t_xyz[7].y + 1.0f) * fY,
                     xyz[7].z, color);
...
void drawOBJ(model *m, float pos_x, float pos_y, float pos_z, float rot_x, float rot_y, float rot_z) {
  ...
  for (int i = 0; i < m->facesCount * 3; i += 3) {
    if (m->texture == NULL) {
      gsKit_prim_triangle_gouraud_3d(gsGlobal, (t_xyz[m->idxList[i]].x + 1.0f) * fX, (t_xyz[m->idxList[i]].y + 1.0f) * fY, xyz[m->idxList[i]].z,
      ...
      gsKit_prim_triangle_goraud_texture_3d_st(
          gsGlobal, m->texture, (t_xyz[m->idxList[i]].x + 1.0f) * fX, (t_xyz[m->idxList[i]].y + 1.0f) * fY, xyz[m->idxList[i]].z,
          tex[m->idxList[i]].s, tex[m->idxList[i]].t, (t_xyz[m->idxList[i + 1]].x + 1.0f) * fX, (t_xyz[m->idxList[i + 1]].y + 1.0f) * fY,
          xyz[m->idxList[i + 1]].z, tex[m->idxList[i + 1]].s, tex[m->idxList[i + 1]].t, (t_xyz[m->idxList[i + 2]].x + 1.0f) * fX,
          (t_xyz[m->idxList[i + 2]].y + 1.0f) * fY, xyz[m->idxList[i + 2]].z, tex[m->idxList[i + 2]].s, tex[m->idxList[i + 2]].t,
          rgba[m->idxList[i]].rgbaq, rgba[m->idxList[i + 1]].rgbaq, rgba[m->idxList[i + 2]].rgbaq);
    }
  }
