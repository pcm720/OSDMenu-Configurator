...
static uint8_t imgThreadStack[4096] __attribute__((aligned(16)));
...
static int lua_fontload(lua_State *L) {
  ...
  if (font == NULL)
    return luaL_error(L, "Error loading font (invalid magic).");
...
static int lua_ftload(lua_State *L) {
  ...
  // Try VFS first when available, so we don't print "file loading failed" for VFS-only paths
  if (vfs_available()) {
    size_t sz = 0;
    const void *buf = vfs_get(fontpath, &sz);
    if (buf && sz > 0)
      fntHandle = fntLoadFromMemory(buf, (int)sz);
  }
  if (fntHandle < 0)
    fntHandle = fntLoadFile(fontpath);
  lua_pushinteger(L, fntHandle);
  return 1;
}

static int lua_ftLoadFromMemory(lua_State *L) {
  if (lua_gettop(L) != 1)
    return luaL_error(L, "wrong number of arguments");
  size_t len = 0;
  const char *data = luaL_checklstring(L, 1, &len);
  int id = fntLoadFromMemory(data, (int)len);
...
static int lua_ftCalcDimensions(lua_State *L) {
  ...
  int w = fntCalcDimensions(fontid, text);
  lua_pushinteger(L, w);
  return 1;
}
...
static const luaL_Reg Font_functions[] = {
    ...
    {"ftInit", lua_ftinit},
    {"ftLoad", lua_ftload},
    {"ftLoadFromMemory", lua_ftLoadFromMemory},
...
static int lua_drawimg(lua_State *L) {
  ...
  Color color = 0x80808080;
  if (argc == 4)
    color = (Color)luaL_checknumber(L, 4);
...
static int lua_drawimg_rotate(lua_State *L) {
  ...
  Color color = 0x80808080;
  if (argc == 5)
    color = (Color)luaL_checknumber(L, 5);
  float width = source->Width;
  float height = source->Height;
  float startx = 0.0f;
  float starty = 0.0f;
...
static int lua_drawimg_scale(lua_State *L) {
  ...
  Color color = 0x80808080;
  if (argc == 6)
    color = (Color)luaL_checknumber(L, 6);
  float startx = 0.0f;
  float starty = 0.0f;
  float endx = source->Width;
  float endy = source->Height;
...
static int lua_drawimg_part(lua_State *L) {
  ...
  Color color = 0x80808080;
  if (argc == 8)
    color = (Color)luaL_checknumber(L, 8);
  float width = source->Width;
  float height = source->Height;
...
static int lua_drawimg_full(lua_State *L) {
  ...
  Color color = 0x80808080;
  if (argc == 11)
    color = (Color)luaL_checknumber(L, 11);
...
static int lua_quad(lua_State *L) {
  ...
  if (argc == 9)
    drawQuad(x, y, x2, y2, x3, y3, x4, y4, color);
...
static const luaL_Reg Graphics_functions[] = {{"drawPixel", lua_pixel},
                                              {"drawRect", lua_rect},
                                              {"drawLine", lua_line},
